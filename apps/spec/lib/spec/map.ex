defmodule Spec.Map do
  ...
  @spec new(Spec.t, Spec.t, non_neg_integer, non_neg_integer | nil, boolean, boolean) :: t
  def new(key_spec, value_spec, min_length \\ 0, max_length \\ nil, distinct? \\ false, conform_keys? \\ false) do
    %__MODULE__{
      key_spec: key_spec,
  ...
  defimpl Spec.Conformable do
    ...
    use Spec.Func
    import Spec.Conformable.Spec.List, only: [distinct_problems: 5]
    ...
    def conform(spec, spec_path, via, value_path, value) when is_map(value) do
      ...
      |> ConformError.add_problems(distinct_problems(spec, spec_path, via, value_path, value), true)
      |> case do
    ...
    @spec conform_key_value(@for.t, [term], [Spec.Ref.t], [term], {term, term})
      :: {:ok, term} | {:error, [ConformError.Problem.t]}
    defp conform_key_value(%@for{conform_keys?: conform_keys?} = spec, spec_path, via, value_path, {k, v}) do
      {
      ...
      |> case do
        ...
        {{:error, kps}, {:error, vps}} -> {:error, vps ++ kps}
        {_, {:error, vps}} -> {:error, vps}
        {{:error, kps}, _} -> {:error, kps}
      end
    end

    @spec size_problems(@for.t, [term], [Spec.Ref.t], [term], term) :: [ConformError.Problem.t]
