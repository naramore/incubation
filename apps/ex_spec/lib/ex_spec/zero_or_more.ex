defmodule ExSpec.ZeroOrMore do
  ...
  defimpl ExSpec.RegexOperator do
    ...
    def conform(_spec, spec_path, via, value_path, value) when is_list(value) do
      {:error, [ConformError.new_problem(&proper_list?/1, spec_path, via, RegexOp.uninit_path(value_path), value)]}
    end
    def conform(_spec, spec_path, via, value_path, value) do
      {:error, [ConformError.new_problem(&is_list/1, spec_path, via, RegexOp.uninit_path(value_path), value)]}
    end

    @spec conform_regex(RegexOperator.t, [term], [ExSpec.Ref.t], [term], term, [term])
    ...
    @spec conform_non_regex(ExSpec.t, [term], [ExSpec.Ref.t], [term], term, non_neg_integer, [term])
